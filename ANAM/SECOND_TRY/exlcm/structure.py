"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class structure(object):
    __slots__ = ["id", "message", "enabled"]

    __typenames__ = ["int64_t", "string", "boolean"]

    __dimensions__ = [None, None, None]

    def __init__(self):
        self.id = 0
        self.message = ""
        self.enabled = False

    def encode(self):
        buf = BytesIO()
        buf.write(structure._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">q", self.id))
        __message_encoded = self.message.encode('utf-8')
        buf.write(struct.pack('>I', len(__message_encoded)+1))
        buf.write(__message_encoded)
        buf.write(b"\0")
        buf.write(struct.pack(">b", self.enabled))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != structure._get_packed_fingerprint():
            raise ValueError("Decode error")
        return structure._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = structure()
        self.id = struct.unpack(">q", buf.read(8))[0]
        __message_len = struct.unpack('>I', buf.read(4))[0]
        self.message = buf.read(__message_len)[:-1].decode('utf-8', 'replace')
        self.enabled = bool(struct.unpack('b', buf.read(1))[0])
        return self
    _decode_one = staticmethod(_decode_one)

    _hash = None
    def _get_hash_recursive(parents):
        if structure in parents: return 0
        tmphash = (0xe69b0646b2c8a6d6) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if structure._packed_fingerprint is None:
            structure._packed_fingerprint = struct.pack(">Q", structure._get_hash_recursive([]))
        return structure._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

